...
def test_post_create_dry_run_and_apply(mock_stores):
    ...
    with (
        patch("wpguard_mcp.tools.blocks.get_site_registry", return_value=registry),
        patch("wpguard_mcp.tools.blocks.get_packet_store", return_value=p_store),
        patch(
            "wpguard_mcp.transports.companion_plugin.call",
            return_value={"post_id": 42, "url": "https://example.com/p/42"},
        ),
    ):
        preview = blocks.wp_post_create(
            "test-plugin", "New Landing Page", "<!-- wp:heading --><h2>Title</h2><!-- /wp:heading -->", apply=False
        )
        assert preview["dry_run"] is True

        applied = blocks.wp_post_create(
            "test-plugin", "New Landing Page", "<!-- wp:heading --><h2>Title</h2><!-- /wp:heading -->", apply=True
        )
