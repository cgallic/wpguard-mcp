...
from wpguard_mcp.guard import PacketRequiredError, PacketStore, SnapshotStore
...
def test_eval_sandbox_requires_packet(mock_stores):
    p_store, s_store, registry = mock_stores
    with (
        patch("wpguard_mcp.tools.eval_sandbox.get_site_registry", return_value=registry),
