...
def wp_magic_login(site: str, user_login: str = "admin", ttl_seconds: int = 600) -> dict:
    ...
    if site_config.transport == "ssh":
        php = (
            f"$u = get_user_by('login', {repr(user_login)}) "
            ...
            "echo json_encode(['login_url' => $url, 'user_id' => $u->ID, "
            f"'user_login' => $u->user_login, 'expires_in' => {ttl_seconds}]);"
        )
        ...
        res = companion_plugin.call(site_config, "magic_login", {"user_login": user_login, "ttl_seconds": ttl_seconds})
